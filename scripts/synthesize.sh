#!/bin/bash
# PSYCHE/OS - Cross-source synthesis (requires all 3 extractions)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/_config.sh"
PROMPT_FILE="$PROMPT_DIR/synthesis.txt"
...
psyche_llm_run "$TMP" "$OUTPUT_DIR/synthesis-unified.json"
psyche_strip_fences "$OUTPUT_DIR/synthesis-unified.json"
rm -f "$TMP"
